...
export class StoreNode<T> implements IStoreNode<T> {
    ...
    constructor(protected config: StoreNodeConfig<T>) {
        ...
        this._storage = config.local ? LocalStorageManager : SessionStorageManager;
        if (config.cache
            && !(config.timeout && this._storage.isTimeout(config.id))) {
            this._data = this._storage.get(config.id);
            this._dataString = this._storage.getNoParse(config.id);
        }
        if (config.joinTo) {
    ...
    public refreshTime() {
        if (this.config.cache) {
            this._storage.refreshTime(this.config.id);
        }
    }
    ...
    private cache() {
        if (this.config.cache) {
            this._storage.setNoStringify(this.config.id, this._dataString);
        }
    }
