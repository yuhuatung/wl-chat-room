...
import { CUI } from '../cui';
import { AjaxHeader, AjaxContentType } from './ajax.enums';

export class AjaxUtil {
    public static readonly ContentTypeJson = new AjaxHeaders(AjaxHeader.ContentType, AjaxContentType.JSON);

    public static readonly AcceptTypeStream = new AjaxHeaders(AjaxHeader.Accept, AjaxContentType.EVENTSTREAM);
    ...
    public static parseSendDataPHP(data: any): string {
        ...
        if (CUI.isObject(data)) {
            ...
            for (let name in data) {
                value = data[name];
                AjaxUtil.parseValuePHP(array, name, value == undefined ? null : value);
            }
            sendString = array.join('&');
        } else if (CUI.isArray(data)) {
            let pair: IAjaxNameValuePair, array = [];
            for (let i in data) {
                pair = data[i];
                AjaxUtil.parseValuePHP(array, pair.name, pair.value == undefined ? null : pair.value);
            }
            sendString = array.join('&');
        } else {
            sendString = data;
    ...
    public static toFormData(data: any): FormData {
        ...
        if (CUI.isObject(data)) {
            ...
            for (let i in data) {
                pair = data[i];
                if (!(pair.value == undefined || pair.value == null)) {
                    AjaxUtil.appendValue(formData, name, pair.value);
                }
            }
        } else {
            formData = data;
